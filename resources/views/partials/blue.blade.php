<section class="section section-padded dark-bg">
    <div class="container">
        <div class="row">
            <div class="col-md-8 col-md-offset-2">
                <div class="owl-twitter owl-carousel">
                    <div class="item text-center">
                        <form action="" method="POST" enctype="multipart/form-data">
                            <h1 class="light">Inscription</h1>
                            <div class="row">
                                <div class="col-lg-5">
                                    <div class="input-group" style='margin: 20px 0'>
                                        <span class="input-group-addon" id="sizing-addon2">Nom</span>
                                        <input type="text" class="form-control" placeholder="Username"
                                            aria-describedby="sizing-addon2">
                                    </div>
                                    <div class="input-group" style='margin: 20px 0'>
                                        <span class="input-group-addon" id="sizing-addon2">Prenom</span>
                                        <input type="text" class="form-control" placeholder="Username"
                                            aria-describedby="sizing-addon2">
                                    </div>
                                    <div class="input-group" style='margin: 20px 0'>
                                        <span class="input-group-addon" id="sizing-addon2">Statut</span>
                                        <input type="text" class="form-control" placeholder="Username"
                                            aria-describedby="sizing-addon2">
                                    </div>
                                    <div class="input-group" style='margin: 20px 0'>
                                        <span class="input-group-addon" id="sizing-addon2">Commune</span>
                                        <input type="text" class="form-control" placeholder="Username"
                                            aria-describedby="sizing-addon2">
                                    </div>
                                    <div class="input-group" style='margin: 20px 0'>
                                        <span class="input-group-addon" id="sizing-addon2">Genre</span>
                                        <input type="radio" class="form-control" placeholder="Username"
                                            aria-describedby="sizing-addon2">
                                    </div>
                                </div>
                                <div class="col-lg-5">
                                    <div class="input-group" style='margin: 20px 0'>
                                        <span class="input-group-addon" id="sizing-addon2">Adresse</span>
                                        <input type="text" class="form-control" placeholder="Username"
                                            aria-describedby="sizing-addon2">
                                    </div>
                                    <div class="input-group" style='margin: 20px 0'>
                                        <span class="input-group-addon" id="sizing-addon2">Email</span>
                                        <input type="text" class="form-control" placeholder="Username"
                                            aria-describedby="sizing-addon2">
                                    </div>
                                    <div class="input-group" style='margin: 20px 0'>
                                        <span class="input-group-addon" id="sizing-addon2">Objectif</span>
                                        <input type="text" class="form-control" placeholder="Username"
                                            aria-describedby="sizing-addon2">
                                    </div>
                                    <div class="input-group" style='margin: 20px 0'>
                                        <span class="input-group-addon" id="sizing-addon2">Image</span>
                                        <input type="file" class="form-control" placeholder="Username"
                                            aria-describedby="sizing-addon2">
                                    </div>
                                    <div class="input-group" style='margin: 20px 0'>
                                        <span class="input-group-addon" id="sizing-addon2">Ordinateur</span>
                                        <input type="checkbox" class="form-control" placeholder="Username"
                                            aria-describedby="sizing-addon2">
                                    </div>
                                    <div class="input-group" style='margin: 20px 0'>
                                        <span class="input-group-addon" id="sizing-addon2">Newsletter</span>
                                        <input type="checkbox" class="form-control" placeholder="Username"
                                            aria-describedby="sizing-addon2">
                                    </div>
                                </div>
                            </div>
                        </form>
                    </div>
                    <div class="item text-center">
                        <form action="">
                            <div class="input-group" style='margin: 20px 0'>
                                <span class="input-group-addon" id="sizing-addon2">Name</span>
                                <input type="text" class="form-control" placeholder="Username"
                                    aria-describedby="sizing-addon2">
                            </div>
                        </form>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>
